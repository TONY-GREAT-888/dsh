...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'ultralytics/cfg/models/v8/yolov8-DualConv-slimneck_Head(+P2).yaml')  # 不使用预训练权重训练
    #model = YOLO(r'yolov8.yaml').load("YOLOv8n.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'ultralytics/cfg/datasets/ship12k.yaml',
        epochs=300,  # (int) 训练的周期数
        patience=50,  # (int) 等待无明显改善以进行早期停止的周期数
        batch=16,  # (int) 每批次的图像数量
        imgsz=640,  # (int) 输入图像的大小，整数或w，h
        save=True,  # (bool) 保存训练检查点和预测结果
        save_period=-1,  # (int) 每x周期保存检查点（如果小于1则禁用）
        cache=False,  # (bool) True/ram、磁盘或False。使用缓存加载数据
        device='',  # (int | str | list, optional) 运行的设备，例如 cuda device=0 或 device=0,YOLOv10n,2,3 或 device=cpu
