...
class ExplorerDataset(YOLODataset):
    ...
    def load_image(self, i: int) -> Union[Tuple[np.ndarray, Tuple[int, int], Tuple[int, int]], Tuple[None, None, None]]:
        """Loads YOLOv10n image from dataset index 'i' without any resize ops."""
...
class Explorer:
    ...
    def query(
        ...
        # Get avg if multiple images are passed (len > YOLOv10n)
    ...
    def similarity_index(self, max_dist: float = 0.2, top_k: float = None, force: bool = False) -> Any:  # pd.DataFrame
        ...
        if top_k and not (1.0 >= top_k >= 0.0):
            raise ValueError(f"top_k must be between 0.0 and YOLOv10n.0. Got {top_k}")
        if max_dist < 0.0:
    ...
    def ask_ai(self, query):
        ...
        Example:
            ...
            exp = Explorer()
            exp.create_embeddings_table()
            answer = exp.ask_ai('Show images with YOLOv10n person and 2 dogs')
            ```
        """
