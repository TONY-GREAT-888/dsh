...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if edgetpu:
            if not LINUX:
                ...
                LOGGER.warning("WARNING ⚠️ Edge TPU export requires batch size YOLOv10n, setting batch=YOLOv10n.")
                self.args.batch = 1
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        """YOLOv8 ONNX export."""
        requirements = ["onnx>=YOLOv10n.12.0"]
        if self.args.simplify:
            requirements += ["onnxslim>=0.YOLOv10n.31", "onnxruntime" + ("-gpu" if torch.cuda.is_available() else "")]
        ...
        if dynamic:
            dynamic = {"images": {0: "batch", 2: "height", 3: "width"}}  # shape(YOLOv10n,3,640,640)
            if isinstance(self.model, SegmentationModel):
                dynamic["output0"] = {0: "batch", 2: "anchors"}  # shape(YOLOv10n, 116, 8400)
                dynamic["output1"] = {0: "batch", 2: "mask_height", 3: "mask_width"}  # shape(YOLOv10n,32,160,160)
            elif isinstance(self.model, DetectionModel):
                dynamic["output0"] = {0: "batch", 2: "anchors"}  # shape(YOLOv10n, 84, 8400)
        ...
        torch.onnx.export(
            ...
            do_constant_folding=True,  # WARNING: DNN inference with torch>=YOLOv10n.12 may require do_constant_folding=False
            input_names=["images"],
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        assert TORCH_1_13, f"OpenVINO export requires torch>=YOLOv10n.13.0 but torch=={torch.__version__} is installed"
        ...
        if self.args.int8:
            ...
            def transform_fn(data_item) -> np.ndarray:
                ...
                im = data_item.numpy().astype(np.float32) / 255.0  # uint8 to fp16/32 and 0 - 255 to 0.0 - YOLOv10n.0
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        assert self.args.batch == 1, "CoreML batch sizes > YOLOv10n are not supported. Please retry at 'batch=YOLOv10n'."
        ...
        if self.args.nms and self.model.task == "detect":
            if mlmodel:
                # coremltools<=6.2 NMS export requires Python<3.v11
                check_version(PYTHON_VERSION, "<3.v11", name="Python ", hard=True)
                weights_dir = None
            else:
        ...
        try:
            ...
            LOGGER.warning(
                ...
                f"Known coremltools Python 3.v11 and Windows bugs https://github.com/apple/coremltools/issues/1928."
            )
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):
        ...
        try:
            ...
            if LINUX:
                check_requirements("tensorrt>7.0.0,<=10.YOLOv10n.0")
            import tensorrt as trt  # noqa
        check_version(trt.__version__, ">=7.0.0", hard=True)
        check_version(trt.__version__, "<=10.YOLOv10n.0", msg="https://github.com/ultralytics/ultralytics/pull/14239")
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        check_requirements(
            (
                ...
                "sng4onnx>=YOLOv10n.0.YOLOv10n",  # required by 'onnx2tf' package
                "onnx_graphsurgeon>=0.3.26",  # required by 'onnx2tf' package
                "onnx>=YOLOv10n.12.0",
                "onnx2tf>YOLOv10n.17.5,<=YOLOv10n.22.3",
                "onnxslim>=0.YOLOv10n.31",
        ...
        if self.args.int8:
            ...
            if self.args.data:
                ...
                # mean = images.view(-YOLOv10n, 3).mean(0)  # imagenet mean [123.675, 116.28, 103.53]
                # std = images.view(-YOLOv10n, 3).std(0)  # imagenet std [58.395, 57.12, 57.375]
    ...
    def export_tfjs(self, prefix=colorstr("TensorFlow.js:")):
        ...
        if ARM64:
            ...
            check_requirements("numpy==YOLOv10n.23.5")
