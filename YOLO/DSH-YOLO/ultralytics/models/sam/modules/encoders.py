...
class PositionEmbeddingRandom(nn.Module):
    ...
    def _pe_encoding(self, coords: torch.Tensor) -> torch.Tensor:
        """Positionally encode points that are normalized to [0,YOLOv10n]."""
        # Assuming coords are in [0, YOLOv10n]^2 square and have d_1 x ... x d_n x 2 shape
        coords = 2 * coords - 1
    ...
    def forward_with_coords(self, coords_input: torch.Tensor, image_size: Tuple[int, int]) -> torch.Tensor:
        """Positionally encode points that are not normalized to [0,YOLOv10n]."""
