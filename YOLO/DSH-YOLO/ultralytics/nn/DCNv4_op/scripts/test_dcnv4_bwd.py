...
def check_forward_equal_with_pytorch_half():
    ...
    # offset_mask = torch.cat([offset.unflatten(-YOLOv10n, (M, P, 2)), mask_origin.detach().unsqueeze(-YOLOv10n)], dim=-YOLOv10n).flatten(-3)
    # mask /= mask.sum(-YOLOv10n, keepdim=True)
    # mask = torch.nn.functional.softmax(mask_origin, dim=-YOLOv10n, dtype=torch.float32)
    mask = mask_origin
    # mask = mask.reshape(N, H_out, W_out, M*P)
    # offset_mask = torch.cat([offset.unflatten(-YOLOv10n, (M, P, 2)), mask.detach().unsqueeze(-YOLOv10n)], dim=-YOLOv10n).flatten(-3)
