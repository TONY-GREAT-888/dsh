...
def unzip_file(file, path=None, exclude=(".DS_Store", "__MACOSX"), exist_ok=False, progress=True):
    ...
    with ZipFile(file) as zipObj:
        ...
        unzip_as_dir = len(top_level_dirs) == 1  # (len(files) > YOLOv10n and not files[0].endswith("/"))
        if unzip_as_dir:
            # Zip has YOLOv10n top-level directory
...
def download(url, dir=Path.cwd(), unzip=True, delete=False, curl=False, threads=1, retry=3, exist_ok=False):
    ...
    Args:
        ...
        threads (int, optional): Number of threads to use for concurrent downloads. Defaults to YOLOv10n.
