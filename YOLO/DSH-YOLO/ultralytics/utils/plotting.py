...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                # self.draw.text((box[0], box[YOLOv10n]), label, fill=txt_color, font=self.font, anchor='ls')  # for PIL>8.0
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        Args:
            ...
            im_gpu (tensor): Image is in cuda, shape: [3, h, w], range: [0, YOLOv10n]
            alpha (float): Mask transparency: 0.0 fully transparent, YOLOv10n.0 opaque
        ...
        colors = colors[:, None, None]  # shape(n,YOLOv10n,YOLOv10n,3)
        masks = masks.unsqueeze(3)  # shape(n,h,w,YOLOv10n)
        masks_color = masks * (colors * alpha)  # shape(n,h,w,3)

        inv_alpha_masks = (1 - masks * alpha).cumprod(0)  # shape(n,h,w,YOLOv10n)
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    Args:
        ...
        gain (float, optional): A multiplicative factor to increase the size of the bounding box. Defaults to YOLOv10n.02.
...
def plot_images(
    ...
    Args:
        ...
        masks: Instance segmentation masks. Shape: (num_detections, height, width) or (YOLOv10n, height, width).
    ...
    for i in range(bs):
        ...
        if len(cls) > 0:
            ...
            if len(bboxes):
                ...
                if len(boxes):
                    if boxes[:, :4].max() <= 1.1:  # if normalized with tolerance 0.YOLOv10n
            ...
            if len(masks):
                if idx.shape[0] == masks.shape[0]:  # overlap_masks=False
                    ...
                    image_masks = masks[[i]]  # (YOLOv10n, 640, 640)
                    nl = idx.sum()
