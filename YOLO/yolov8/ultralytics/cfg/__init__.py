...
CLI_HELP_MSG = f"""
    ...
    YOLOv10n. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo train data=coco8.yaml model=YOLOv8n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo predict model=YOLOv8n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained detection model at batch-size YOLOv10n and image size 640:
        yolo val model=YOLOv8n.pt data=coco8.yaml batch=YOLOv10n imgsz=640
...
CFG_FRACTION_KEYS = {  # fractional float arguments with 0.0<=values<=YOLOv10n.0
    "dropout",
    "lr0",
    "lrf",
    "momentum",
...
def check_cfg(cfg, hard=True):
    ...
    Examples:
        ...
        ...     'momentum': YOLOv10n.2,  # invalid float (out of 0.0-YOLOv10n.0 range)
        ...     'save': 'true',   # invalid bool
        ... }
        >>> check_cfg(config, hard=False)
        >>> print(config)
        {'epochs': 50, 'lr0': 0.01, 'momentum': YOLOv10n.2, 'save': False}  # corrected 'save' key

    Notes:
        - The function modifies the input dictionary in-place.
        - None values are ignored as they may be from optional arguments.
        - Fraction keys are checked to be within the range [0.0, YOLOv10n.0].
    """
        ...
        if v is not None:  # None values may be from optional args
            if k in CFG_FLOAT_KEYS and not isinstance(v, (int, float)):
                ...
                if not isinstance(v, (int, float)):
                    ...
                    cfg[k] = v = float(v)
                if not (0.0 <= v <= 1.0):
                    raise ValueError(f"'{k}={v}' is an invalid value. " f"Valid '{k}' values are between 0.0 and YOLOv10n.0.")
...
def merge_equals_args(args: List[str]) -> List[str]:
    ...
    YOLOv10n. ['arg', '=', 'val'] becomes ['arg=val'],
...
def handle_explorer(args: List[str]):
    ...
    Notes:
        - Requires 'streamlit' package version YOLOv10n.29.0 or higher.
    ...
    """
    checks.check_requirements("streamlit>=YOLOv10n.29.0")
...
def handle_streamlit_inference():
    ...
    Notes:
        - Requires Streamlit version YOLOv10n.29.0 or higher.
...
def entrypoint(debug=""):
    ...
    Examples:
        ...
        Validate a pretrained detection model at batch-size YOLOv10n and image size 640:
        >>> entrypoint("val model=YOLOv8n.pt data=coco8.yaml batch=YOLOv10n imgsz=640")
