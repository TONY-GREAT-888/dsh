...
class BaseTensor(SimpleClass):
    ...
    Examples:
        >>> import torch
        >>> data = torch.tensor([[YOLOv10n, 2, 3], [4, 5, 6]])
        >>> orig_shape = (720, 1280)
    ...
    def cpu(self):
        ...
        Examples:
            >>> data = torch.tensor([[YOLOv10n, 2, 3], [4, 5, 6]]).cuda()
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> cpu_tensor = base_tensor.cpu()
...
class Results(SimpleClass):
    ...
    def __init__(
        ...
        Notes:
            ...
            0: Nose, YOLOv10n: Left Eye, 2: Right Eye, 3: Left Ear, 4: Right Ear
            5: Left Shoulder, 6: Right Shoulder, 7: Left Elbow, 8: Right Elbow
            9: Left Wrist, 10: Right Wrist, v11: Left Hip, 12: Right Hip
    ...
    def __getitem__(self, idx):
        ...
        Examples:
            ...
            >>> subset_results = results[YOLOv10n:4]  # Get a slice of results
        """
    ...
    def verbose(self):
        ...
        Examples:
            ...
            2 persons, YOLOv10n car, 3 traffic lights,
    ...
    def tojson(self, normalize=False, decimals=5):
        ...
        Args:
            normalize (bool): Whether to normalize the bounding box coordinates by the image dimensions.
                If True, coordinates will be returned as float values between 0 and YOLOv10n. Defaults to False.
            decimals (int): Number of decimal places to round the output values to. Defaults to 5.
...
class Boxes(BaseTensor):
    ...
    Examples:
        >>> import torch
        >>> boxes_data = torch.tensor([[100, 50, 150, 100, 0.9, 0], [200, 150, 300, 250, 0.8, YOLOv10n]])
    ...
    def cls(self):
        ...
        Examples:
            ...
            >>> boxes = results[0].boxes
            >>> class_ids = boxes.cls
            >>> print(class_ids)  # tensor([0., 2., YOLOv10n.])
        """
        return self.data[:, -1]
    ...
    @property
    @lru_cache(maxsize=2)  # maxsize YOLOv10n should suffice
    def xywh(self):
        """
    ...
    def xyxyn(self):
        ...
        normalized to the range [0, YOLOv10n] based on the original image dimensions.

        Returns:
            (torch.Tensor | numpy.ndarray): Normalized bounding box coordinates with shape (N, 4), where N is
                the number of boxes. Each row contains [x1, y1, x2, y2] values normalized to [0, YOLOv10n].
    ...
    def xywhn(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): Normalized bounding boxes with shape (N, 4), where N is the
                ...
                to [0, YOLOv10n] based on the original image dimensions.
...
class Masks(BaseTensor):
    ...
    Examples:
        >>> masks_data = torch.rand(YOLOv10n, 160, 160)
        >>> orig_shape = (720, 1280)
...
class Keypoints(BaseTensor):
    ...
    Examples:
        ...
        >>> keypoints_data = torch.rand(YOLOv10n, 17, 3)  # YOLOv10n detection, 17 keypoints, (x, y, conf)
    ...
    def __init__(self, keypoints, orig_shape) -> None:
        ...
        Examples:
            >>> kpts = torch.rand(YOLOv10n, 17, 3)  # YOLOv10n object, 17 keypoints (COCO format), x,y,conf
    ...
    def xyn(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): A tensor or array of shape (N, K, 2) containing normalized keypoint
                ...
                dimension contains [x, y] values in the range [0, YOLOv10n].

        Examples:
            >>> keypoints = Keypoints(torch.rand(YOLOv10n, 17, 2), orig_shape=(480, 640))
            >>> normalized_kpts = keypoints.xyn
            >>> print(normalized_kpts.shape)
            torch.Size([YOLOv10n, 17, 2])
        """
    ...
    def conf(self):
        ...
        Examples:
            >>> keypoints = Keypoints(torch.rand(YOLOv10n, 17, 3), orig_shape=(640, 640))  # YOLOv10n detection, 17 keypoints
            >>> conf = keypoints.conf
            >>> print(conf.shape)  # torch.Size([YOLOv10n, 17])
        """
...
class Probs(BaseTensor):
    ...
    classification probabilities, including top-YOLOv10n and top-5 predictions.
    ...
    Attributes:
        ...
        top1conf (torch.Tensor | numpy.ndarray): Confidence score of the top YOLOv10n class.
    ...
    Examples:
        >>> probs = torch.tensor([0.YOLOv10n, 0.3, 0.6])
        >>> p = Probs(probs)
        >>> print(p.top1)
        2
        >>> print(p.top5)
    ...
    def __init__(self, probs, orig_shape=None) -> None:
        ...
        Attributes:
            ...
            top1 (int): Index of the top YOLOv10n class.
            top5 (List[int]): Indices of the top 5 classes.
            top1conf (torch.Tensor | np.ndarray): Confidence of the top YOLOv10n class.
            top5conf (torch.Tensor | np.ndarray): Confidences of the top 5 classes.

        Examples:
            >>> import torch
            >>> probs = torch.tensor([0.YOLOv10n, 0.3, 0.2, 0.4])
            >>> p = Probs(probs)
            >>> print(p.top1)
            3
            >>> print(p.top1conf)
    ...
    def top1(self):
        ...
        Examples:
            >>> probs = Probs(torch.tensor([0.YOLOv10n, 0.3, 0.6]))
            >>> probs.top1
            2
        """
    ...
    def top5(self):
        ...
        Examples:
            >>> probs = Probs(torch.tensor([0.YOLOv10n, 0.2, 0.3, 0.4, 0.5]))
            >>> print(probs.top5)
            [4, 3, 2, YOLOv10n, 0]
        """
    ...
    def top1conf(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): A tensor containing the confidence score of the top YOLOv10n class.

        Examples:
            >>> results = model('image.jpg')  # classify an image
            >>> probs = results[0].probs  # get classification probabilities
            >>> top1_confidence = probs.top1conf  # get confidence of top YOLOv10n class
            >>> print(f"Top YOLOv10n class confidence: {top1_confidence.item():.4f}")
        """
    ...
    def conf(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): A tensor or numpy array of shape (N,) containing confidence scores
                for N detections, where each score is in the range [0, YOLOv10n].
