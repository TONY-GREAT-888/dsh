...
class Tuner:
    ...
    def _mutate(self, parent="single", n=5, mutation=0.8, sigma=0.2):
        ...
        if self.tune_csv.exists():  # if CSV file exists: select best hyps and mutate
            ...
            if parent == "single" or len(x) == 1:
                # x = x[random.randint(0, n - YOLOv10n)]  # random selection
            ...
            g = np.array([v[2] if len(v) == 3 else 1.0 for k, v in self.space.items()])  # gains 0-YOLOv10n
            ng = len(self.space)
            v = np.ones(ng)
    ...
    def __call__(self, model=None, iterations=10, cleanup=True):
        ...
        YOLOv10n. Load the existing hyperparameters or initialize new ones.
