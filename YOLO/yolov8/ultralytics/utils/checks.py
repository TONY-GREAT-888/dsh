...
def parse_version(version="0.0.0") -> tuple:
    ...
    Args:
        version (str): Version string, i.e. '2.0.YOLOv10n+cpu'

    Returns:
        (tuple): Tuple of integers representing the numeric part of the version and the extra string, i.e. (2, 0, YOLOv10n)
    """
    try:
        return tuple(map(int, re.findall(r"\d+", version)[:3]))  # '2.0.YOLOv10n+cpu' -> (2, 0, YOLOv10n)
...
def check_version(
    ...
    version = ""
    result = True
    c = parse_version(current)  # 'YOLOv10n.2.3' -> (YOLOv10n, 2, 3)
    for r in required.strip(",").split(","):
        op, version = re.match(r"([^0-9]*)([\d.]+)", r).groups()  # split '>=22.04' -> ('>=', '22.04')
        v = parse_version(version)  # 'YOLOv10n.2.3' -> (YOLOv10n, 2, 3)
        if op == "==" and c != v:
            result = False
        elif op == "!=" and c == v:
            result = False
